...
%% 详细验证一个例子
disp('单个编码示例验证:');
test_msg = [1 0 1 0]; % 测试消息
disp('原始消息:');
disp(test_msg);

% 编码
test_codeword = linear_block_encoder(test_msg, G);
disp('编码后的码字:');
disp(test_codeword);

% 添加单比特错误
error_pos = 3;
test_received = test_codeword;
test_received(error_pos) = ~test_received(error_pos);
disp('接收到的码字 (添加了位置3的错误):');
disp(test_received);

% 译码
test_decoded = linear_block_decoder(test_received, H, G);
disp('译码后的消息:');
disp(test_decoded);

% 验证是否纠正了错误
if all(test_msg == test_decoded)
    disp('成功纠正错误!');
else
    disp('未能纠正错误.');
end 